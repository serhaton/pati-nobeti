...
async function cleanupUserReferences(userId: string): Promise<void> {
  const operations: Array<Promise<{ error: any }>> = [
    ...
    supabase.from('communities').update({ approved_by: null }).eq('approved_by', userId),
    ...
    supabase.from('expenses').update({ approved_by: null }).eq('approved_by', userId),
    ...
    supabase.from('contributions').update({ approved_by: null }).eq('approved_by', userId),
